...
const BooksPage = () => {
  ...
  const toggleGenre = (selectedGenre) => {
    setGenre((prev) => {
      const updatedGenres = prev.includes(selectedGenre)
  ...
  const fetchBooks = async () => {
    ...
    try {
      const response = await axios.get(`${import.meta.env.VITE_URL}/books`, {
        params: { page, limit, search, genre: genre.join(" ") },
  ...
  return (
    <div className="relative p-4 lg:p-10 z-0">
        ...
        {genresList.map((g) => (
          <button
            ...
            className={`p-1 lg:px-4 lg:py-2 rounded-md ${
              genre.includes(g) ? "bg-blue-500 text-white" : "bg-gray-200"
            }`}
          >
            {g}
          </button>
      ...
      {loading ? (
        ...
        <div className="text-gray-700 text-xl w-[100%] text-center">
          No Books Found! :(
        </div>
      ) : (
        <>
